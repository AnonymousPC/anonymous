...
{
    if constexpr ( decay<decltype(right)>::dimension() == 1 or decay<decltype(right)>::dimension() == -1 )
        return array<string,decay<decltype(right)>::dimension()> ( right.size(), [&] ( int i ) { return stringalizer(right[i]); } );
    else
        return array<string,decay<decltype(right)>::dimension()> ( right.row(), [&] ( int i ) { return stringalize_array(left,right[i],stringalizer); } );
}
...
void aux::print_array ( auto& left, const auto& right, int depth /* =1 */ )
{
    ...
    if constexpr ( decay<decltype(right)>::dimension() == 1 or decay<decltype(right)>::dimension() == -1 )
        ...
        for ( int i in range ( right.row() ) )
        {
            ...
            {
                left << ' ';
                for ( int _ in range ( decay<decltype(right)>::dimension() - 1 ) )
                    left << '\n';
