...
namespace detail
{
    ...
    constexpr array_upper<type,1,device>::array_upper ( array<type,2,device>& init_host, array_attribute, int init_ofs )
    ...
    template < class type, class device >
    constexpr array_upper<type,1,device>::array_upper ( const array<type,2,device>& init_host, array_attribute, int init_ofs )
    ...
    {
        return get_host().get_value(get_offset(), offset);
    }
    ...
    {
        return get_host().get_pointer(get_offset(), offset);
    }
    ...
    {
        return get_attribute() == slice_attribute ? get_host<1>().template get_rows   <dim-1>(get_offset()).begin() otherwise
    ...
    {
        return get_attribute() == slice_attribute ? get_host<1>().template get_rows   <dim-1>(get_offset()).end() otherwise
    ...
    {
        return get_attribute() == slice_attribute ? get_host<1>().get_size_top() otherwise
    ...
    constexpr int array_upper<type,dim,device>::get_size_axis ( ) const
    {
        ...
        return get_attribute() == slice_attribute ? get_host<1>().template get_size_axis<axis+1>() otherwise
