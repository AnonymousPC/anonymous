...
{
    using type = std::conditional<index==0,typename input_type::key_type,typename input_type::value_type>::type;
};

template < int index, ap::pair_type input_type >
    requires ( index == 0 or index == 1 )
constexpr decltype(auto) ap::get ( input_type& p )
{
...
template < int index, ap::pair_type input_type >
    requires ( index == 0 or index == 1 )
constexpr decltype(auto) ap::get ( const input_type& p )
{
    if constexpr ( index == 0 )
        return p.key();
    else
...
template < int index, ap::tuple_type input_type >
    requires ( index >= 0 and index <= input_type::size() - 1 )
constexpr decltype(auto) ap::get ( input_type& t )
{
...
template < int index, ap::tuple_type input_type >
    requires ( index >= 0 and index <= input_type::size() - 1 )
constexpr decltype(auto) ap::get ( const input_type& t )
{
    return t.template value<index+1>();
}
